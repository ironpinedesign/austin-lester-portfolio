...
export async function studioIdentity(){const user=await getRequestIdentity();if(!user)return {user:null,isOwner:false,claimed:false};const d=await db();const owner=await d.prepare('SELECT user_id FROM studio_owner WHERE id=1').first<{user_id:string}>();return {user,isOwner:owner?.user_id===user.userId,claimed:!!owner};}
